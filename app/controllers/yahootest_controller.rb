      ...
      if (products)
        products.each do |index, productJSON|
          if (index.to_i.to_s == index.to_s)
            product = Product.new(name: productJSON["Name"], category_id:1, description: productJSON["Description"], image: productJSON["Image"]["Medium"], price_currency: productJSON["Price"]["_attributes"]["currency"], price_value: productJSON["Price"]["_value"], review_count:productJSON["Review"]["Count"], review_rate:productJSON["Review"]["Rate"])
            product.save
          end
        end
      end
